    ...
    (function init(jQuery, window, document, undefined) {
        ...
        FLIPBOOK.Main = function(options, elem) {
            ...
            this.bodyHasVerticalScrollbar = function() {
                return self.$body.height() > self.$window.height()
            }
            ...
            if (options.lightBox) {
                ...
                //  self.lightboxPreloader = jQuery('<div class="flipbook-lightbox-preloader">Loading...</div>')

                jQuery(this).bind('lightboxLoadingStarted', function() {

                    // self.preloader.addClass('flipbook-lightbox-preloader')
                    //jQuery('body').append(self.lightboxPreloader)

                })

                jQuery(this).bind('lightboxOpened', function() {

                    // self.preloader.removeClass('flipbook-lightbox-preloader')
            ...
            function init() {
                ...
                if (self.options.fillPreloader.enabled) {
                    ...
                    self.setFillPreloaderProgress(.1)
            ...
            function initSound() {
                ...
                // self.flipsound = new Audio(self.options.assets.flipMp3)
            ...
            if (this.options.lightBox) {
                ...
                this.preloader.appendTo(jQuery('body')).css('position', 'fixed');
        ...
        FLIPBOOK.Main.prototype = {
            ...
            start: function() {
                ...
                this.setFillPreloaderProgress(.7)
            ...
            updateCurrentPage: function(index) {
                ...
                if (this.options.singlePageMode) {
                    ...
                    this.enableNext(rightIndex < this.options.pages.length - 1)
                    //this.enableAutoplay(rightIndex < this.options.pages.length - 1)

                    this.currentPageNumber = rightIndex
                    this.setHash(this.currentPageNumber + 1)

                    this.cPage = [this.currentPageNumber]
                ...
                if (this.Book.singlePage) {
                    //rightIndex++;
                    s = rtl ? this.options.pages.length - rightIndex : rightIndex + 1;

                    this.currentPage.attr('value', s)
                    this.currentPage.text(s + ' / ' + total)
            ...
            initJpg: function() {
                ...
                this.loadPage(0, this.options.pageTextureSize, function() {
                    ...
                    self.setFillPreloaderProgress(.5)
                    ...
                    self.loadPage(1, self.options.pageTextureSize, function() {
                        ...
                        if (!self.options.pageWidth)
                            self.options.pageWidth = parseInt(1000 * p1.width / p1.height)
                        if (!self.options.pageHeight)
                            self.options.pageHeight = 1000
            ...
            initPdf: function(loadAllPages) {
                ...
                if (this.options.viewMode == "swipe") this.options.textLayer = true

                this.$fillPreloader ? this.setFillPreloaderProgress(.1) : this.setLoadingProgress(.1)
                ...
                self.setFillPreloaderProgress(.2)

                if (!FLIPBOOK.scriptsLoaded[FLIPBOOK.pdfServiceSrc] && typeof FLIPBOOK.PdfService == 'undefined') {
                ...
                self.setFillPreloaderProgress(.3)

                //PDFJS.disableWorker = this.options.disableWorker || false;
                PDFJS.externalLinkTarget = PDFJS.LinkTarget.BLANK
                PDFJS.workerSrc = this.options.pdfjsworkerSrc || FLIPBOOK.pdfjsworkerSrc
                ...
                PDFJS.getDocument({
                    ...
                    self.options.pageLoaded = function(index, size) {
                        if (self.Book.onPageLoaded)
                            self.Book.onPageLoaded(index, size)
                    }
                    ...
                    self.options.pageUnloaded = function(arr) {
                    ...
                    self.pdfService.init(function() {
                        ...
                        // self.loadPage(0, self.options.pageTextureSize, function(){

                        self.start()

                        // })

                    }, loadAllPages) // load entire pdf text content if search is enabled
                });
            },
            ...
            loadPage: function(index, size, callback) {
                ...
                if (this.options.pdfMode) {
                    ...
                    if (!this.options.pages[index].img) {

                        // console.log("......loading page ",index , " ", size)

                        this.options.pages[index].img = new Image()
                        // console.log('loading page ' + index)
                        this.options.pages[index].img.setAttribute("id", index);

                        this.options.pages[index].img.onload = function() {

                            // setTimeout(function(){
                            //     self.pageLoaded({ index: index, size: size, image: self.options.pages[index].img }, callback)

                            // },7000)
                            self.options.pages[index].imgLoaded = true

                            var $htmlContentDiv = jQuery(document.createElement('div')).addClass('flipbook-page-htmlContent')

                            jQuery(self.options.pages[index].htmlContent).appendTo($htmlContentDiv)



                            self.pageLoaded({ index: index, size: size, image: self.options.pages[index].img, htmlContent: $htmlContentDiv }, callback)

                        }

                        this.options.pages[index].img.src = this.options.pages[index].src

                    } else if (this.options.pages[index].imgLoaded) {

                        // console.log("page "+index+" already loaded")
            ...
            loadThumbFromPdf: function(i, arr) {
                ...
                this.pdfDocument.getPage(i + 1).then(function(page) {
                    ...
                    page.render(renderContext).then(function() {
                        page.cleanup()
                        if ((page.pageIndex + 1) < self.pdfDocument.pdfInfo.numPages)
            ...
            findInDocument: function(str) {
                ...
                // console.log("find in document : ",str)

                this.hideAllThumbs()
                this.$findInfo.hide()
                this.pagesFound = 0

                this.$findInfo.show().text('0 pages found containing "' + str + '"')
            ...
            findInPage: function(str, index) {
                ...
                this.pdfService.findInPage(str, index, function(matches) {
                    if (matches > 0) {
                        ...
                        self.pagesFound++;
                        self.$findInfo.show().text(self.pagesFound + ' pages found containing "' + str + '"')
            ...
            scrollPageIntoView: function(pageNumber) {
                ...
                pageNumber = this.options.rightToLeft ? this.options.pages.length - pageNumber + 1 : pageNumber
            ...
            createBook: function() {
                ...
                if (!FLIPBOOK.scriptsLoaded[FLIPBOOK.iscrollSrc] && typeof THREE == 'undefined') {
                    self.loadScript(FLIPBOOK.iscrollSrc, self.createBook)
                    return;
                }

                self.setFillPreloaderProgress(.9)
                ...
                if (self.options.viewMode == "webgl") {
                    ...
                    if (!FLIPBOOK.scriptsLoaded[FLIPBOOK.flipBookSwipeSrc] && typeof FLIPBOOK.BookSwipe == 'undefined') {
                        self.loadScript(FLIPBOOK.flipBookSwipeSrc, self.createBook)
                        return;
                    }
                } else {
                    if (!FLIPBOOK.scriptsLoaded[FLIPBOOK.flipbookBook3Src] && typeof FLIPBOOK.Book3 == 'undefined') {
            ...
            onBookCreated: function() {
                ...
                document.addEventListener("keydown", function(e) {
                    ...
                    if (!self.isZoomed() && self.bodyHasVerticalScrollbar() && !self.options.lightBox && !self.fullscreenActive && self.options.arrowsDisabled) return
                ...
                this.bookLayer.bind('DOMMouseScroll', function(e) {
                    if (!self.Book.enabled) return;
                    if (!self.isZoomed() && self.bodyHasVerticalScrollbar() && !self.options.lightBox && !self.fullscreenActive && self.options.wheelDisabled) return
                    e.stopPropagation()
                    e.preventDefault()
            ...
            initSwipe: function() {
                ...
                window.jQuery(this.bookLayer).swipe({
                        ...
                        if ((phase == "end" || phase == "cancel") && duration < 200 && distance < 10) {
                            ...
                            if (self.clickTimer == null) {
                                ...
                                if (self.zoom >= self.options.zoomMax)
                                    self.zoomTo(self.options.zoomMin, 0, e)
                                else
                                    self.zoomTo(self.options.zoomMax, 0, e)
                            ...
                            if ((direction == "up" || direction == "down") && phase == 'move' || self.zoom > 1) return;
                    ...
                    pinchStatus: function(event, phase, direction, distance, duration, fingerCount, pinchZoom) {
                        ...
                        else if (self.options.viewMode == "webgl")
                            self.zoomTo(self.zoomStart * pinchZoom, 0)
            ...
            zoomOut: function(e) {
                ...
                this.zoomTo(this.zoom, 0, e)

                // this.Book.zoomTo(this.zoom, 50, e);
            ...
            zoomIn: function(e) {
                ...
                // console.log("this.book.zoomIn("+this.zoom+")")

                // this.Book.zoomTo(this.zoom, 0, e);
            ...
            createBookmark: function() {
                ...
                this.bookmarkHolder = jQuery('<div>')
                    .addClass('flipbook-tocHolder skin-color-bg flipbook-bmHolder flipbook-font')
                    .appendTo(this.wrapper)
                    .hide()
                // .css('left', '-1000px')
                ...
                var left = jQuery('<a><div class="c-l-p skin-color flipbook-btn">Bookmark left page</div></a>')
                    .appendTo(this.bookmark)
                    .bind('touchend click', function(e) {
                        self.bookmarkPage(self.cPage[0], this)
                        e.preventDefault()
                ...
                var right = jQuery('<a><div class="c-r-p skin-color flipbook-btn">Bookmark right page</div></a>')
                    .appendTo(this.bookmark)
                    .bind('touchend click', function(e) {
            ...
            createMenuHeader: function(elem, title, onClose) {
                ...
                var btnClose = jQuery('<span>')
                    .attr('aria-hidden', 'true')
                    .appendTo(header)
                    .addClass('flipbook-btn-close fa fa-times flipbook-icon-fa skin-color')
                    .bind('touchend click', function(e) {
                        e.stopPropagation();
                        e.preventDefault();
                        onClose.call(self)
            ...
            createToc: function(tocArray) {
                ...
                this.createMenuHeader(this.toc, "Table of content", this.toggleToc)
                        ...
                        var tocItem = jQuery(document.createElement('a'))
                            ...
                            .bind('touchend click', function(e) {
                                ...
                                if (!self.tocScroll.moved) {
                                    var targetPage = Number(jQuery(this).attr('title'));

                                    targetPage = self.options.rightToLeft ? self.options.pages.length - targetPage + 1 : targetPage


                                    if (self.Book.goingToPage != targetPage) {
            ...
            createThumbs: function() {
                ...
                this.createMenuHeader(this.thumbHolder, "Thumbnails", this.toggleThumbs)
            ...
            togglePrintMenu: function() {
                ...
                if (!this.printMenu) {
                    ...
                    this.createMenuHeader(content, "Print", this.togglePrintMenu)

                    var current = jQuery('<a><div class="c-p skin-color flipbook-btn">Print current page</div></a>')
                        .appendTo(content)
                        .bind('touchend click', function(e) {
                            self.printPage(self.cPage[0], this)
                        })
                    var left = jQuery('<a><div class="c-l-p skin-color flipbook-btn">Print left page</div></a>').appendTo(this.printMenu)
                        .appendTo(content)
                        .bind('touchend click', function(e) {
                            self.printPage(self.cPage[0], this)
                        })

                    var right = jQuery('<a><div class="c-r-p skin-color flipbook-btn">Print right page</div></a>').appendTo(this.printMenu)
                        .appendTo(content)
                        .bind('touchend click', function(e) {
                            self.printPage(self.cPage[1], this)
                        })

                    var all = jQuery('<a><div class="skin-color flipbook-btn">Print all pages</div></a>')
                        .appendTo(content)
                        .bind('touchend click', function(e) {
            ...
            toggleDownloadMenu: function() {
                ...
                if (!this.dlMenu) {
                    ...
                    this.createMenuHeader(content, "Download", this.toggleDownloadMenu)

                    var current = jQuery('<a><div class="c-p skin-color flipbook-btn">Download current page</div></a>')
                        .appendTo(content)
                        .bind('touchend click', function(e) {
                            self.downloadPage(self.cPage[0], this)
                        })
                    var left = jQuery('<a><div class="c-l-p skin-color flipbook-btn">Download left page</div></a>')
                        .appendTo(content)
                        .bind('touchend click', function(e) {
                            self.downloadPage(self.cPage[0], this)
                        })

                    var right = jQuery('<a><div class="c-r-p skin-color flipbook-btn">Download right page</div></a>')
                        .appendTo(content)
                        .bind('touchend click', function(e) {
                            self.downloadPage(self.cPage[1], this)
                        })

                    var all = jQuery('<a><div class="skin-color flipbook-btn">Download all pages</div></a>')
                        .appendTo(content)
                        .bind('touchend click', function(e) {

                            var link = document.createElement('a');
                            link.href = self.options.btnDownloadPages.url;
                            link.download = self.options.btnDownloadPages.name;
                            link.dispatchEvent(new MouseEvent('click'));
            ...
            toggleAutoplay: function(value) {
                ...
                if (this.autoplay) {
                    this.menuWrapper.find('.fa-play').removeClass('fa-play').addClass('fa-pause')
                    ...
                    this.menuWrapper.find('.fa-pause').removeClass('fa-pause').addClass('fa-play')
